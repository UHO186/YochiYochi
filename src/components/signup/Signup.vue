<template>
    <div>
        <div>
            <Input names="이름" placeholders="ex) 홍길동" v-model="value.name" :valids="valid.nameHasError" :passwords="false" />
        </div>
        <div>
            <Input names="E-mail" placeholders="ex) kream@kream.co.kr" v-model="value.email" :valids="valid.emailHasError"
                :passwords="false" /><button c>중복체크</button>
            <!-- <Input names="E-mail" placeholders="ex) kream@kream.co.kr" v-model="value.email" :valids="true" /> -->
        </div>
        <div>
            <!-- Boolean 값만 넣어주고 싶다면 true, false로만 -->
            <Input names="비밀번호" placeholders="영문, 숫자, 특수문자 조합 8-16자" v-model="value.password" :passwords="true"
                :valids="valid.passwordHasError" />
        </div>
        <div>
            <Input names="비밀번호 확인" v-model="value.password_confirmation" :passwords="true"
                :valids="valid.passworConfirmdHasError" />
        </div>
        <div>
            <Input names="전화번호" placeholders="ex) 010-0000-1111" v-model="value.tel" :passwords="false"
